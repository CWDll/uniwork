...
- [ ] 이미 가입된 이메일 안내에서 비밀번호 재설정 화면으로 이동할 수 있다.
- [ ] `/forgot-password`에서 비밀번호 재설정 메일 요청 화면이 열린다.
- [ ] `/signup?role=company`에서 사업자등록번호와 사업자등록증 파일을 받을 수 있다.
- [ ] 사업자등록증은 PDF/JPG/JPEG/PNG, 5MB 이하 안내와 파일명 표시/다시 업로드 UI가 보인다.
...
- [ ] `/company/settings`에서 사업자등록증 파일을 제출할 수 있고 제출 여부가 보인다.
...
- [ ] `/admin/companies`에서 기업이 제출한 사업자등록증 제출 여부와 확인 링크가 보인다.
